...
**The data**: We will use a subset of the `vehicles` data from the `fueleconomy` dataset throughout. These are fuel economy data from the [EPA](https://www.fueleconomy.gov/feg/download.shtml). Some variables have been excluded or modified from the original dataset. For more details search for `vehicles` in the Help section or run `?vehicles` in the console.
